	...
	morphMap.forEach(function(element,i){
		...
		li.appendTo(form);
             ...
             $('.'+i+'Checkbox').change(function() {
                 if(this.checked) {
                     $('.MORPH_'+i+'_'+this.id).css('background',colores_google(morphMap.get(i).indexOf(this.id))+"44");
                 }
                 else{
                     $('.MORPH_'+i+'_'+this.id).css('background',"none");
                 }
             });
        });
    });
